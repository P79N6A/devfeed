<template>
    <div id="sourceList">
        <div class="alert alert-dismissible" :class="{ 'alert-success': status, 'alert-danger': !status }" role="alert"
             v-show="msg">
        ...
        <div class="alert alert-info" role="alert" v-if="!hasResult">
            <p>暂无数据</p>
        </div>
        <table class="table table-bordered" v-if="hasResult">
            ...
            <tr v-for="(row, index) in result.data">
                ...
                <td>{{ row.tags }}</td>
                <td>
                    <button @click="publish(index)" class="btn btn-xs btn-warning" v-bind:disabled="row.onqueue">发布
                    </button>
        ...
        <ul v-show="result.total > 1" class="pagination">
            <li v-if="result.prev_page_url"><a href="#" @click.prevent="fetch(result.current_page-1)" rel="prev">上一页</a>
            </li>
            <li v-else class="disabled"><span>上一页</span></li>
            <li class="disabled">
                <span>{{ result.current_page }} / {{ parseInt(result.total / result.per_page) + 1 }}</span></li>
            <li v-if="result.next_page_url"><a href="#" @click.prevent="fetch(result.current_page+1)" rel="next">下一页</a>
            </li>
