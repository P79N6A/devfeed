            ...
            <ul class="list clearfix"><!--通过类名list、item进行列表展示方式的切换-->
                ...
                @foreach($articleDetail as $item)
                    <li>
                        ...
                        <h3>{{ link_to(route('front.article.view', $item->id), $item->title) }}<a href="{{ $item->source_url }}" class="origin-link"><i class="spr"></i></a><span class="read-all spr">{{ $item->click_count }}</span></h3>
